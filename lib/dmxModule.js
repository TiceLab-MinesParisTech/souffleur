var DmxModule = function(server) {
	...
	var Dmx = require("dmx");
	this.dmx = new Dmx();
	this.universe = this.dmx.addUniverse("output", "null");
...
DmxModule.prototype.resolveChannel = function(channel) {
	if (typeof channel == "string") {
		return this.def.channels[channel];
	}
	return channel;
};
...
DmxModule.prototype.bindEvents = function(socket) {
	...
	socket.emit("dmx::faders::define", this.def);
};
...
DmxModule.prototype.setFader = function(ref, value) {
	if (ref in this.mapping) {
		...
		var value = Math.floor((value - mapping.min) / (mapping.max - mapping.min) * 255);
